...
const DonorLogin = (props) => {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post("http://localhost:5000/donor_login", values)
      .then((res) => {
  ...
  const handleForClick = (event) => {
    ...
    if (passwordEvent.current.value !== confirmEvent.current.value) {
      ...
      axios
        .post("http://localhost:5000/donor_fgt_pass", values)
        .then((res) => {
          if (res.data.Status === "Success" && res.data.message!=="(Rows matched: 0  Changed: 0  Warnings: 0") {
        ...
        <div className={style.container}>
          <div className={style.popup_inner}>
            <div className={style.b}>
              ...
              <h1 className={style.header}>DONOR LOGIN</h1>
            </div>
            ...
            <Form method="POST" onSubmit={handleSubmit} className={style.box}>
              ...
              {forgotTrue ? (
                ...
                <p onClick={handleOnForgot} className={style.forgotPass}>Forgot Password?</p>
              )}
              {forgotTrue ? (
