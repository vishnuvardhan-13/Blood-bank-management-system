...
import EmployeeLogin from "./EmployeeLoginPage";
...
const BloodBankLogin = (props) => {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post("http://localhost:5000/blood_bank_login", values)
      .then((res) => {
  ...
  const handleForClick = (event) => {
    ...
    if (passwordEvent.current.value !== confirmEvent.current.value) {
      ...
      axios
        .post("http://localhost:5000/bb_fgt_pass", values)
        .then((res) => {
        ...
        <div className={style.container}>
          <div className={style.popup_inner}>
            <div className={style.b}>
              ...
              <h1 className={style.header}>BLOOD BANK LOGIN</h1>
            </div>
