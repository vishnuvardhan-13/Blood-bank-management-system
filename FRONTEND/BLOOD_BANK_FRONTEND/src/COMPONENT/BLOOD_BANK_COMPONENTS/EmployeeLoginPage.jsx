...
const EmployeeLogin = (props) => {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post("http://localhost:5000/employee_login", values)
      .then((res) => {
        if (res.data.Status === "Success") {
          console.log("Success");
          navigate("/blood_bank")
          props.setELogpopup(false)
        }
      })
      .catch((err) => {
  ...
  const handleForClick = (event) => {
    ...
    if (passwordEvent.current.value !== confirmEvent.current.value) {
      ...
      axios
        .post("http://localhost:5000/emp_fgt_pass", values)
        .then((res) => {
        ...
        <div className={style.container}>
          <div className={style.popup_inner}>
            <div className={style.b}>
              ...
              <h1 className={style.header}>EMPLOYEE LOGIN</h1>
            </div>
