...
const PatientRegister = (props) => {
  ...
  const handleSubmit = (event) => {
    ...
    const values = {
      ...
      patient_name: nameEvent.current.value.toUpperCase(),
      age: ageEvent.current.value,
      gender: genderEvent.current.value,
      patient_num: mobileEvent.current.value,
    ...
    axios
      .post("http://localhost:5000/patient_signup", values)
      .then((res) => {
        if (res.data.Status === "Success") {
          props.setPpopup(false);
        } else {
          alert("Error");
        }
      })
